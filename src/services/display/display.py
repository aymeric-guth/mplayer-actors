    ...
    def dispatch(self, sender: int, msg: Message) -> None:
        ...
        match msg:
            ...
            case Event(type="property-change", name=name, args=args):
                ...
                if self.playback_overlay:
                    send(
                        self.child,
                        Request(
                            type="render", name="playback", args=self.media_meta.copy()
                        ),
                    )
            ...
            case Event(type="io", name="resize"):
                ...
                if self.files_overlay:
                    send(
                        self.child,
                        Request(
                            type="render", name="files", args=[dir_list, files_list]
                        ),
                    )
                if self.playback_overlay:
                    send(
                ...
                if self.cmd_overlay:
                    send(
                        self.child,
                        Request(type="render", name="cmd", args=self.cmd_buff),
                    )
            ...
            case Event(type="error", name="bad-cmd", args=args):
                send(
                    to=self.child, what=Request(type="render", name="popup", args=args)
                )
