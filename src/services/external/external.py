        ...
        match msg:
            ...
            case Request(type="smb", name="ping", args=args):
                try:
                    ...
                    self.logger.error(
                        f"Could not join host: {SMB_ADDR} on port: {SMB_PORT} cause: {err}"
                    )
                    send(
                        to=self.pid,
                        what=Request(type="smb", name="ping", args=args + 1),
                    )
                else:
                    self.logger.info(
                        f"host: {SMB_ADDR} on port: {SMB_PORT} is up, trying to connect"
                    )
            ...
            case Request(type="cmd", name="hook", args=args) if isinstance(
                args, list | tuple
            ) and len(args) == 2:
                k, v = args
                try:
            ...
            case Request(type="cmd", name="jump", args=args):
                try:
                    ...
                    send(
                        to="Files",
                        what=Request(type="files", name="cwd-change", args=cwd),
                    )
